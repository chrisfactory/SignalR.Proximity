...
{      
    ...
    public class ClientProxy : IDisposable
    {
        ...
        private SignalRProximityClientConfiguration _config;
        ...
        /// </param>
        /// <param name="config">
        ///     A <see cref="SignalRProximityClientConfiguration"/> that represents the configuration of <see cref="ClientProxy"/> type. 
        /// </param>
        public ClientProxy(HubConnection cnx, ScopeDefinitionBase scope, IRetryPolicy retryPolicy, SignalRProximityClientConfiguration config)
        {
            _config = config;
            _hubConnection = cnx;
            ...
            lock (_sync)
            {
                ...
                if (groups != null)
                {
                    ...
                    if (oldCount != newCount && _hubConnection.State == HubConnectionState.Connected)
                        _hubConnection.InvokeAsync("Interact", new SignalRProximityRequest() { Scope = ClientScopeDefinition.JoinGroups(groups) }, new object[0]);
                }
            }
        }
        ...
        public void QuitGroups(params string[] groups)
        {
            lock (_sync)
            {
                ...
                if (groups != null)
                {
                    ...
                    newCount = _groups.Count;
                    if (oldCount != newCount && _hubConnection.State == HubConnectionState.Connected)
                        _hubConnection.InvokeAsync("Interact", new SignalRProximityRequest() { Scope = ClientScopeDefinition.QuitGroups(groups) }, new object[0]);
                }
            }
        }
            ...
            if (_config.AutoRestoredGroups)
                lock (_sync)
                {
                    if (_groups.Count > 0 && _hubConnection.State == HubConnectionState.Connected)
                        return _hubConnection.InvokeAsync("Interact", new SignalRProximityRequest() { Scope = ClientScopeDefinition.JoinGroups(_groups.ToArray()) }, new object[0]);
                }
